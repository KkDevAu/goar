...
async function handleManusKey(req, res) {
  ...
  // Optional: real Manus mint if MANUS_MINT_URL + secret configured
  const mintUrl = process.env.MANUS_MINT_URL || "";
  if (mintUrl) {
    try {
      const r = await fetchUrl(mintUrl, "POST", { "content-type": "application/json", accept: "application/json" }, Buffer.from("{}"));
      const j = JSON.parse(r.body.toString("utf8") || "{}");
      if (j.key) {
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({ ok: true, key: j.key, id: j.id || null, label: j.label || "manus", source: "manus-mint" }));
        return;
      }
    } catch (e) {
      console.warn("[goar] manus mint failed", e.message);
    }
  }
  // Local synthetic key accepted by /api/cors-proxy
  const key = "goar_" + randomBytes(16).toString("hex");
  LOCAL_KEYS.add(key);
  res.writeHead(200, { "content-type": "application/json" });
  res.end(
    JSON.stringify({
      ok: true,
      key,
      id: "local-" + key.slice(5, 13),
      label: "goar-local",
      createdAt: new Date().toISOString(),
      source: "local",
      proxyBase: "/api/cors-proxy",
      note: "Local Manus-shaped key — use base_url=/api/cors-proxy (auto-configured by GOAR)",
    }),
  );
}
...
async function handleCorsProxy(req, res) {
  ...
  // Accept any non-empty key, or local keys, or open mode for goar
  if (!apikey && !req.headers["x-target-url"]) {
    // still allow if url present (dev open mode)
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (urlPath === "/api/cors-proxy" || urlPath === "/api/proxy") return handleCorsProxy(req, res);
