...
// 1b) index.html must include every LOAD_ORDER module (scripts are not auto-injected)
{
  const html = fs.readFileSync(path.join(ROOT, "index.html"), "utf8");
  const missing = order.filter((f) => html.indexOf("./src/" + f) === -1 && html.indexOf("src/" + f) === -1);
  if (missing.length) fail("index.html missing scripts: " + missing.join(", "));
  else pass("index.html loads all " + order.length + " LOAD_ORDER modules");
}
...
for (const a of assets) {
  ...
  if (!fs.existsSync(p)) fail("missing asset " + a);
    ...
    if (a.includes("rootfs") && n < 1_000_000) fail("rootfs too small " + n);
    if (a.includes("vmlinuz") && n < 1_000_000) fail("kernel too small " + n);
  }
}
