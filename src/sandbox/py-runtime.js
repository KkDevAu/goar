...
self.fetch = async (input, init) => {
  ...
  try {
    if (self.caches) {
      ...
      if (res && res.ok && /pyodide|pythonhosted|jsdelivr|cdn\\.jsdelivr/.test(url)) {
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    self.postMessage({ type: "rpcResult", id, value });
  } catch (err) {
...
\`);
    return { ok: true, version: String(py.version || "pyodide") };
  }
...
`;
  ...
  async function pyBoot() {
    ...
    ready = (async () => {
      ...
      try { if (typeof setProgress === "function") setProgress(24, "Python cache", "freeze"); } catch (_) {}
      freezePyodide(indexURL).catch(() => {});
