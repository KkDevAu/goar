...
(function () {
  ...
  function goarShowView(view) {
    ...
    const labels = { chat: "Chat", computer: "Computer", ide: "Files", skills: "Skills" };
  ...
  async function loadFileList() {
    ...
    const guestUp = (typeof envReady !== "undefined" && !!envReady) || !!(typeof window !== "undefined" && window.__emulator);
    if (typeof toolLs !== "function" || !guestUp) {
    ...
    try {
      ...
      list.innerHTML = '<div class="files-empty">Guest disk is still coming up. <button type="button" id="files-retry" class="ide-tool-btn">Retry</button></div>';
      document.getElementById("files-retry")?.addEventListener("click", () => loadFileList());
      if (!loadFileList._retry) {
  ...
  async function openFile(path) {
    ...
    try {
      ...
      if (ed) ed.value = String(raw || "");
      ideOriginal = ed ? ed.value : "";
  ...
  async function saveFile() {
    ...
    try {
      if (typeof toolWrite === "function") await toolWrite({ path: idePath, content: $("ide-editor").value });
      ideOriginal = $("ide-editor").value;
      if ($("ide-status-dirty")) $("ide-status-dirty").textContent = "";
