...
(function wireCredGate() {
  ...
  go?.addEventListener("click", () => {
    ...
    const apiBase = (b && b.value) || "";
    const apiKey = (k && k.value) || "";
    const apiModel = (document.getElementById("credModel")?.value || "").trim();
    if (!apiModel && provider !== "ollama") return;
    try { saveSettings({ provider, apiBase, apiKey, apiModel }); } catch (_) {}
    if (typeof reflectActiveModel === "function") reflectActiveModel(apiModel, provider);
