(function () {
  ...
  const PACK_LOCAL = "./goar.pack.zip";
  const PACK_REMOTE = "https://cdn.jsdelivr.net/gh/KkDevAu/goar@main/goar.pack.zip";
  const CSS = [
    "src/css/xterm.css",
    "src/css/app.css",
  ...
  async function fetchPack() {
    const urls = [PACK_LOCAL, PACK_REMOTE];
    let last = null;
    for (const url of urls) {
      try {
        const res = await fetch(url, { cache: "force-cache" });
        if (!res.ok) { last = new Error(url + " " + res.status); continue; }
