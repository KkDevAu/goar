...
async function preflightApi() {
  try {
    ...
    let modelsOk = false;
    try {
      const r = await fetch(modelsUrl(base, provider), { headers: { Authorization: headers.Authorization || "" } });
      modelsOk = r.ok;
    } catch (_) {}
    const cr = await fetch(chatCompletionsUrl(base, provider), {
      method: "POST",
      headers,
      body: JSON.stringify({
...
function fillSettingsForm() {
  const s = ensureDefaultSettings();
  let pid = s.provider || detectProvider(s.apiBase) || "openrouter";
  if (pid === "custom") pid = "openai-compatible";
  if (el.provider) {
    el.provider.value = getProvider(pid) ? pid : "openai-compatible";
  }
...
function saveSettingsFromForm() {
  ...
  const s = { provider: p ? p.id : provider, apiBase, apiModel, apiKey, customDns };
  saveSettings(s);
  return s;
}
